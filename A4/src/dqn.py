...
class DQNTrainer:
    def __init__(
        ...
        self.optimizer = optim.AdamW(self.policy_net.parameters(), lr=params.LR, amsgrad=True) # Initialize the optimizer
        self.memory = memory
        self.device = device
        self.params = params
    ...
    def select_action(self, state_tensor: torch.Tensor) -> torch.Tensor:
        ...
        eps_threshold = self.params.EPS_END + (self.params.EPS_START - self.params.EPS_END) * \
                    math.exp(-1. * self.steps_done / self.params.EPS_DECAY) # TODO: Implement epsilon decay based on the provided equation
    ...
    def optimize_model(self) -> None:
        ...
        with torch.no_grad():
            next_state_value[non_final_mask] = self.target_net(non_final_next_states).max(1)[0].detach()
    ...
    def soft_update(self) -> None:
        ...
        **Implementation Details:**
        ...
        for key in policy_dict.keys():
            target_dict[key] = self.params.TAU * policy_dict[key] + (1 - self.params.TAU) * target_dict[key]
